...
NUM_COLORS_BACKGROUND = 7  # Количество цветов для фона
MIN_AREA_CONTOUR = 30  # Минимальная площадь полигона (в пикселях)
MAX_DIMENSION = 500  # Максимальный размер изображения для обработки (в пикселях)
...
FONT_COLOR = (0, 0, 0)  # Цвет шрифта для номеров цветов (RGB)
CONTOUR_COLOR = (0, 0, 0)  # Цвет контуров на контурном изображении (RGB)
...
def create_paint_by_numbers(image_path):
    ...
    background_smooth = cv2.pyrMeanShiftFiltering(
        background, MEAN_SHIFT_RADIUS, MEAN_SHIFT_COLOR
    )
    ...
    _, main_labels, main_centers = cv2.kmeans(
        main_pixel_values,
        NUM_COLORS_MAIN,
        None,
        criteria,
    ...
    _, background_labels, background_centers = cv2.kmeans(
        background_pixel_values,
        NUM_COLORS_BACKGROUND,
        None,
        criteria,
        10,
        cv2.KMEANS_RANDOM_CENTERS,
    ...
    background_labeled_image = background_centers[background_labels.flatten()].reshape(
        background.shape
    )
    ...
    for i, color in enumerate(all_centers):
        ...
        for contour in contours:
            if cv2.contourArea(contour) >= MIN_AREA_CONTOUR:
                cv2.drawContours(
                    blank_image, [contour], -1, CONTOUR_COLOR, CONTOUR_THICKNESS
                )
    ...
    ax[2].set_title("Outline with Main Objects")
    ax[2].axis("off")
